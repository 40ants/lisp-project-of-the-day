...
- 27 March - [[content/2020/03/0020-cl-hash-table-destructuring.org][cl-hash-table-destructuring]]
        ...
        ((write-paragraph ()
           (cl-who:with-html-output (stream)
             (:p (cl-who:str
                  ...
                  ;; links to the images
                  (replace-images
                   (cl-strings:join (nreverse buffer)
                                    :separator " ")))))
         ...
         (process (line)
           (cond
             ((and (cl-strings:starts-with line
                                           "- ")
                   (not reading-quote))
              (push (subseq line 2)
                    buffer)
...
(defun strip-doc-folder (filename)
  ...
  (cond
    ((cl-strings:starts-with filename "docs/")
     (subseq filename 5))
    (t filename)))
...
(defun get-title-for-index (file)
  (let* ((title (get-title file))
         ...
         (number (first (cl-strings:split only-file #\-))))
    (values title number)))
      ...
      (app-page (stream)
        (:ul :style "list-style: none"
         (loop for file in (reverse files)
               for url = (strip-doc-folder (make-output-filename file))
